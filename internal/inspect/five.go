...
type Opts5[Type EightBits] struct {
...
// Performs inspection.
func (opts *Opts5[Type]) Do() (Result[Type], error) {
...
func (opts *Opts5[Type]) main() Result[Type] {
...
func (opts *Opts5[Type]) loop(firsts chan int64) Result[Type] {
	result := Result[Type]{}
			...
			for third := opts.min; third <= opts.max; third++ {
				for fourth := opts.min; fourth <= opts.max; fourth++ {
					for fifth := opts.min; fifth <= opts.max; fifth++ {
						reference, fault := opts.Reference5(
							first,
							second,
							third,
							fourth,
