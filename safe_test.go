...
func TestIsMaxNegative(t *testing.T) {
	require.True(t, IsMaxNegative[int8](-128))
	require.False(t, IsMaxNegative[int8](-127))
	require.False(t, IsMaxNegative[int8](-126))
	require.False(t, IsMaxNegative[int8](-125))
	require.False(t, IsMaxNegative[int8](-3))
	require.False(t, IsMaxNegative[int8](-2))
	require.False(t, IsMaxNegative[int8](-1))
	require.False(t, IsMaxNegative[int8](0))
	require.False(t, IsMaxNegative[int8](1))
	require.False(t, IsMaxNegative[int8](2))
	require.False(t, IsMaxNegative[int8](3))
	require.False(t, IsMaxNegative[int8](124))
	require.False(t, IsMaxNegative[int8](125))
	require.False(t, IsMaxNegative[int8](126))
	require.False(t, IsMaxNegative[int8](127))

	require.False(t, IsMaxNegative[uint8](0))
	require.False(t, IsMaxNegative[uint8](1))
	require.False(t, IsMaxNegative[uint8](2))
	require.False(t, IsMaxNegative[uint8](3))
	require.False(t, IsMaxNegative[uint8](255))
}

func TestIsMaxPositive(t *testing.T) {
	require.False(t, IsMaxPositive[int8](-128))
	require.False(t, IsMaxPositive[int8](-127))
	require.False(t, IsMaxPositive[int8](-126))
	require.False(t, IsMaxPositive[int8](-125))
	require.False(t, IsMaxPositive[int8](-3))
	require.False(t, IsMaxPositive[int8](-2))
	require.False(t, IsMaxPositive[int8](-1))
	require.False(t, IsMaxPositive[int8](0))
	require.False(t, IsMaxPositive[int8](1))
	require.False(t, IsMaxPositive[int8](2))
	require.False(t, IsMaxPositive[int8](3))
	require.False(t, IsMaxPositive[int8](124))
	require.False(t, IsMaxPositive[int8](125))
	require.False(t, IsMaxPositive[int8](126))
	require.True(t, IsMaxPositive[int8](127))

	require.False(t, IsMaxPositive[uint8](0))
	require.False(t, IsMaxPositive[uint8](1))
	require.False(t, IsMaxPositive[uint8](2))
	require.False(t, IsMaxPositive[uint8](3))
	require.True(t, IsMaxPositive[uint8](255))
}
...
func FuzzProduct(f *testing.F) {
	f.Fuzz(
		func(t *testing.T, first int16, second int16) {
			...
			product, err := ProductInt(first, second)
			if higher > math.MaxInt16 || higher < math.MinInt16 {
				require.Error(t, err)
				return
			}
			...
			require.NoError(t, err)
			require.Equal(t, higher, int64(product))
		},
	)
}
...
func FuzzProductUnsigned(f *testing.F) {
	f.Fuzz(
		func(t *testing.T, first uint16, second uint16) {
			...
			require.NoError(t, err)
			require.Equal(t, higher, uint64(product))
		},
	)
}
...
func FuzzFloatToInt(f *testing.F) {
	f.Fuzz(
		func(t *testing.T, float float32) {
			if float > float32(math.MaxInt64) || float < float32(math.MinInt64) {
				return
			}

			higher := int64(float)

			casted, err := FloatToInt[float32, int16](float)
			if higher > math.MaxInt16 || higher < math.MinInt16 {
				require.Error(t, err)
				return
			}
...
func FuzzFloatToIntUnsigned(f *testing.F) {
	f.Fuzz(
		func(t *testing.T, float float32) {
			...
			if float > float32(math.MaxUint64) {
				return
			}

			higher := uint64(float)

			casted, err := FloatToInt[float32, uint16](float)
			if higher > math.MaxUint16 {
				require.Error(t, err)
				return
			}
			...
			require.NoError(t, err)
			require.Equal(t, higher, uint64(casted))
		},
	)
}
...
func FuzzInvert(f *testing.F) {
	f.Fuzz(
		func(t *testing.T, number int16) {
			higher := int64(number)

			inverted, err := Invert(number)
			if higher > math.MaxInt16 || higher < math.MinInt16 {
				require.Error(t, err)
				return
			}
			...
			require.NoError(t, err)
			require.Equal(t, -higher, int64(inverted))
		},
	)
}
